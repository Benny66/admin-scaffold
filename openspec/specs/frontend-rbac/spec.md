# frontend-rbac Specification

## Purpose
把「后端 RBAC 已在 `router.go` 逐条接线、权限码已随登录返回，但前端菜单不过滤、按钮不隐藏、无权限路由无兜底」这一断链补上：侧边栏菜单从路由声明派生并按权限过滤，无权限的操作按钮从 DOM 中移除，无权限路由跳转 403、未匹配路由渲染 404；并把「前端用到的权限码必须在后端注册过」编译成会失败的 guard 测试。
## Requirements
