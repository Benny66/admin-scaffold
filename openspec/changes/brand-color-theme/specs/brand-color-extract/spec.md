...
- `--el-color-primary-light-3`：主色与白色按 3:7 混合。
- `--el-color-primary-light-5`：主色与白色按 1:1 混合。
- `--el-color-primary-light-7`：主色与白色按 7:3 混合。
- `--el-color-primary-light-8`：主色与白色按 4:1 混合。
- `--el-color-primary-light-9`：主色与白色按 9:1 混合。
- `--el-color-primary-dark-2`：主色与黑色按 2:8 混合。
- `--brand-from`：主色本身（登录页渐变起点）。
- `--brand-to`：主色与黑色按 3:7 混合（登录页渐变终点）。
...
**理由**：桌面端与移动端登录页的渐变起点本来就不同色（`#1f6feb` vs `#1989fa`）。若回退时统一注入 `#1f6feb`，移动端会在「无 logo」这一默认配置下发生视觉变化，本 change 的「无破坏性 / 零配置开箱即用」目标即不成立；同理，注入 `--el-color-primary: #1f6feb` 会把 EP 默认的 `#409eff` 按钮色一并改掉。交给各端自己的 CSS fallback，回退结果才与现状逐像素一致。
...
`frontend/src/utils/colorExtract.js` 与 `mobile/src/utils/colorExtract.js` MUST 提供相同的导出接口（`extractDominantColor`、`deriveThemeVars`、`applyThemeVars`），两端 store 的 `applyBrandTheme` action 行为一致。
