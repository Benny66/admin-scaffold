...
- **THEN** 后端启动正常，但 `mp-login` 返回 500 + `{ message: "微信小程序登录未配置" }`，给出明确指引
...
- **WHEN** 本 change 落地后检查 `deps.yaml` 的 `backend:` 段
...
- **WHEN** 后端启动并枚举公开路由组
- **THEN** `/api/auth/login` 与 `/api/auth/mp-login` 同在公开组（无需 token）；`/api/auth/logout`、`/api/auth/refresh` 在受保护组（与本变更前一致）

### Requirement: 冒烟测试覆盖 mp-login

`backend/scripts/smoke.sh` MUST 扩展冒烟用例：在 admin 登录 + 受保护路由命中之外，新增 mock `mp-login` 链路（用配置的测试 appid/secret + 占位 code 调用，断言后端响应 400 而非 500，证明接口可达且未配置时优雅降级）。

#### Scenario: mp-login 接口可达性冒烟

- **WHEN** 执行 `make smoke`
- **THEN** 包含一步 `curl -sf -X POST /api/auth/mp-login -d '{"code":"smoke-test"}'`，断言响应 `code === 400`（未配置或 code 非法）而非 500，证明接口已注册
