...
桌面端（`frontend/src/views/Login.vue`）MUST 采用分栏布局：左栏为背景展示区（背景层 + scrim + 品牌块），右栏为登录表单区；视口宽度 < 900px 时 MUST 折叠为单列（左栏隐藏，背景铺满，表单卡片居中）。

#### Scenario: 宽屏渲染分栏

- **WHEN** 视口宽度 ≥ 900px 且已加载系统信息
- **THEN** 页面呈现左右两栏，左栏含背景层与品牌块，右栏含登录表单
...
两端登录页 MUST 在底部渲染 `<AppFooter>`，内容取 `app.footer`；`footer` 为空时 MUST 不渲染占位。移动端 MUST 处理底部安全区。
